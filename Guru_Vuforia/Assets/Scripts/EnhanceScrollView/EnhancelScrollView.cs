...
public class EnhancelScrollView : MonoBehaviour
{
    ...
    private List<Image> textureTargets;

    // 当前处于中间的item
    public static EnhanceItem centerItem;
    public static EnhanceItem preCenterItem;
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (Input.mousePosition.x - a > 150)
            {
                //Debug.LogError(Input.mousePosition.x - a);
                OnBtnRightClick();
            }
            if (Input.mousePosition.x - a < -150)
            {
    ...
    /// 用于层级对比接口
    /// </summary>
    public class CompareDepthMethod : IComparer<UnityEngine.UI.Image>
    {
