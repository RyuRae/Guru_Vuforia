...
namespace Content
{
    public class DeclarativeContent : IContent {
        ...
        {
            if (trackers[3].OnGetCurrentTransform().name.ToLower().Equals("the"))
                trackers.Sort((p, q) => q.GetViewportPos().x.CompareTo(p.GetViewportPos().x));
            if (trackers[1].OnGetChildTransform() == null) return;
            ...
            var anim = trackers[1].OnGetCurrentTransform().GetComponentInChildren<Animation>();
        ...
        private void OnMarked(Transform parent)
        {
            if (point != null)
            {
                if (question.activeSelf) point.SetActive(false);
